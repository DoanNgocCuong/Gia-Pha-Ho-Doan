...
#### 8. Sửa lỗi co giãn ngang quá mức ở các ô Đời 9, 10
- Vấn đề: Một số ô Đời 9, 10 có văn bản dài bị dàn hàng ngang và ô bị kéo giãn to rộng bất thường, làm ô mỏng dính.
- Nguyên nhân: `.node .nm` dùng `flex-direction: row` làm trình duyệt trả về `scrollWidth` là độ dài cả dòng chưa wrap, đánh lừa hàm JS tự động tăng `width` của node lên.
- Giải pháp: Chuyển `.node .nm` thành `flex-direction: column` để ép buộc các từ xếp dọc, giúp đo `scrollWidth` chính xác theo từ dài nhất.
- **Tệp:** [`index.html`](index.html)
