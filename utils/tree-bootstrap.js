 ...
 * @deprecated Runtime dùng tree-bootstrap-v2.js (xem index.html và utils/README.md mục 9).
 ...
 *   - tree-text.js    (calls `normalizeAllNodeLabels`, `fitNodeText`)
...
import { loadPrintSizeConfig, applyPrintConfigToCss, giaPhaLog } from './print-config.js';
import { normalizeNodeLabel, normalizeAllNodeLabels, fitNodeText } from './tree-text.js';
import { scheduleDrawTreeEdges, drawTreeEdges } from './tree-edges.js';
import {
...
import { initTreePan } from './tree-pan.js';
...
function createTreeNodeElement(node, nodeId) {
    ...
    span.className   = 'nm';
    span.textContent = normalizeNodeLabel(node.name || '');
...
async function bootstrapTree() {
    ...
    try {
        const config = await loadPrintSizeConfig();
